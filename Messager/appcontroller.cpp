                                ...
                                const QString &content)
{
    ...
    // 先确保外键对应的 peer 已经存在。
    if (!m_database.upsertPeer(normalizedPeerId,
                               normalizedName,
                               normalizedIp,
                               true)) {
        reportError(
            QStringLiteral("更新用户信息失败：")
            + m_database.lastError());
        return;
    }
    ...
    // 暂时不代表对方一定已经收到。
    m_privateChat.sendMessageToUser(
        normalizedIp,
        normalizedContent);

    if (!m_database.saveMessage(normalizedPeerId,
                                true,
                                normalizedContent)) {
        reportError(
            QStringLiteral("保存发送消息失败：")
            + m_database.lastError());
        return;
    }
...
void AppController::synchronizeOnlineUsers()
{
    ...
    const QVariantList onlineUsers =
        m_privateChat.onlineUsers();

    for (const QVariant &item : onlineUsers) {
        const QVariantMap networkUser = item.toMap();

        const QString name =
            networkUser.value(QStringLiteral("name"))
                .toString()
                .trimmed();

        const QString ip =
            networkUser.value(QStringLiteral("ip"))
                .toString()
                .trimmed();
        ...
        // 当前协议没有 UUID，第一阶段使用 IP 作为 peerId。
...
void AppController::handleMessageReceived(
    ...
    const QString &message)
{
    ...
    // 当前阶段使用 IP 作为 peerId。
    if (!m_database.upsertPeer(normalizedIp,
                               displayName,
                               normalizedIp,
                               true)) {
        reportError(
            QStringLiteral("保存消息发送者失败：")
            + m_database.lastError());
        return;
    }

    if (!m_database.saveMessage(normalizedIp,
                                false,
                                normalizedMessage)) {
        reportError(
            QStringLiteral("保存接收消息失败：")
            + m_database.lastError());
        return;
    }
    ...
    if (m_currentPeerId == normalizedIp) {
        refreshMessages();
    }
}
...
{
    const QVariantList loadedPeers =
        m_database.loadPeers();
...
void AppController::refreshMessages()
{
    ...
    if (!m_currentPeerId.isEmpty()) {
        loadedMessages =
            m_database.loadMessages(m_currentPeerId);
    }
